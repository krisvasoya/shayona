...
import { getBuyerDetail } from '../../services/api';
import { BuyerDetailData, Invoice } from '../../types';

interface BuyerDetailScreenProps {
  navigation: any;
  route: any;
}

export const BuyerDetailScreen: React.FC<BuyerDetailScreenProps> = ({
  navigation,
  route,
}) => {
  ...
  const [data, setData] = useState<BuyerDetailData | null>(null);
  ...
  const fetchDetail = useCallback(async () => {
    try {
      if (!buyerId) return;
      const res = await getBuyerDetail(buyerId);
      setData(res.data);
    } catch (error) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{buyer?.name || t('buyerDetails')}</Text>
        <TouchableOpacity
          style={[styles.newBillBtn, { backgroundColor: '#7C3AED' }]}
          onPress={() =>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.profileSection}>
            ...
            <View style={styles.contactCard}>
              <View style={[styles.avatar, { backgroundColor: '#F3E8FF', borderColor: '#E9D5FF' }]}>
              ...
              <View style={styles.contactInfo}>
                ...
                {buyer?.address ? (
                  <View style={styles.detailRow}>
                    ...
                    <Text style={styles.detailText}>{buyer.address}</Text>
                  </View>
                ) : null}
              </View>
            </View>
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            ...
            <TouchableOpacity
              style={[styles.addFirstBillBtn, { backgroundColor: '#F3E8FF', borderColor: '#DDD6FE' }]}
              onPress={() =>
                navigation.navigate('CreateEditInvoice', {
                  defaultType: 'purchase',
                  partyId: buyer?.id,
            ...
            >
              <Text style={[styles.addFirstBillText, { color: '#7C3AED' }]}>+ Create First Purchase Bill</Text>
            </TouchableOpacity>
          </View>
        }
      />
